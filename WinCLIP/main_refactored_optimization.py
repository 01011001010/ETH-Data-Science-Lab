...
def evaluate_model(model, tokenizer, preprocess, config, device):
    ...
    dataloader = DataLoader(dataset, batch_size=config['batch_size'], num_workers=8, shuffle=False)
    ...
    for data in tqdm(dataloader, desc="Eval"):
        ...
        if isinstance(image[0], list):  # Tiled images case
            batch_size = len(image)
            num_tiles = len(image[0])

            # Flatten the batch to treat each tile as a batch element
            tile_batch = torch.cat([torch.stack(image[i]) for i in range(batch_size)], dim=0)
            tile_scores = compute_image_score(model, text_features, tile_batch)

            # Reshape to batch_size x num_tiles and calculate mean score per image
            tile_scores = tile_scores.view(batch_size, num_tiles)
            avg_scores = tile_scores.mean(dim=1).tolist()  # Average across tiles for each image
            print(f'scores: {avg_scores}')
        else:
            image = image.to(device)
            single_image_score = compute_image_score(model, text_features, image)[0]
            instance_scores.append(single_image_score)

        if config["shot"] == 0:
            # Zero-shot classification
            scores.extend(instance_scores)  # Append all individual scores
            ground_truths.extend([has_anomaly[0].item()] * len(instance_scores))  # Replicate label for each tile/image
        else:
            # Few-shot classification
            combined_image = prepare_few_shot_images(image, ref_images, device)

            # Forward pass with combined images (image + reference images)
            anomaly_map, batch_vis_probs = model.forward(image=combined_image)

            # Calculate avg score combining tile and visual anomaly scores
            for tile_score, vis_prob in zip(instance_scores, batch_vis_probs):
                avg_score = (vis_prob + tile_score) / 2 if not math.isinf(tile_score) else 0
                scores.append(avg_score)
                ground_truths.append(has_anomaly[0].item())  # Append label for each tile/image
